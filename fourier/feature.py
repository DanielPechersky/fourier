...
class FourierSlice(Feature):
    ...
    def plot(self):
        ...
        (pts_avg_artist,) = self.ax.plot(
            pts_avg.real, pts_avg.imag, "go", label="Average of all points"
        )
        self.ax.legend()
    ...
    def update(self):
        self.freq_set_to = (
            int(
                self.freq_set_to
                + math.copysign(
                    self.scroll_amount**2 * self.freq.size / self.steps,
                    -self.scroll_amount,
                )
            )
            % self.freq.size
...
class FourierFeature(Feature):
    ...
    def get_points(self):
        ...
        return np.vectorize(
            lambda freq_: (self.amp * partial_wrap(freq=freq_)).sum() / len(self.time)
        )(self.freq)
...
class FrequencySummed(FourierFeature):
    def plot(self):
        ...
        ax.plot(self.freq, np.hypot(pts.real, pts.imag), c=(0.75, 0, 0.75))
...
class FeatureManager:
    ...
    def add_feature(self, feature: type, *args, **kwargs):
        self.features.append(
            feature(
                self.freq, self.time, self.amp, self.fig, *self.grid, *args, **kwargs
            )
        )
